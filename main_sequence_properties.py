...
def luminosity_MS(mass):
    ...
    lum_ms = (0.39704170 * np.power(mass, 5.5) + 8.52762600 * np.power(mass, 11)) / (
        0.00025546
        + np.power(mass, 3)
        + 5.43288900 * np.power(mass, 5)
        + 5.56357900 * np.power(mass, 7)
        + 0.78866060 * np.power(mass, 8)
...
def radius_MS(mass):
    ...
    radius_ms = (
        ...
        + 10.08855000 * np.power(mass, 11)
        + 1.01249500 * np.power(mass, 19)
        + 0.07490166 * np.power(mass, 19.5)
    ) / (
        0.01077422
        + 3.08223400 * np.power(mass, 2)
...
def effective_temperature(mass=None, lum=None, radius=None):
    """Effective temperature as a function of mass (assuming main sequence) or optionally luminosity and radius"""
...
def Q_ionizing(mass, energy_eV=13.6):
    ...
    ionizing_frac = planck_integral(x1, np.inf)
    return (
        ionizing_frac * L * 1.7e44 / (1 + 3 / x1 - 2 * (1 + x1) / (2 + x1 * (2 + x1)))
    )


def ionizing_frac_approx(x):
