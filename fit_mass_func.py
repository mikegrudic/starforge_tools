...
def massfunc_chabrier(m, log_mpeak, sigma, alpha):
    mbreak = 1.
    mpeak = 10**log_mpeak
    if m>mbreak: massfunc = 1/mbreak * np.exp(-np.log10(mbreak/mpeak)**2 / (2*sigma**2)) * (m/mbreak)**alpha
    else: massfunc = (1/m * np.exp(-np.log10(m/mpeak)**2 / (2*sigma**2)))
    return massfunc

    

def FitMassFunction(masses,mmin=None,mmax=None,form='chabrier', walkers=100,chainlength=1000,burnin=100, return_samples=False):
    ...
    if form=='parravano':
        ...
        def log_prob(x,mstar):
            mpeak, sigma, alpha = x
            if sigma < 0.1: return -np.inf
            if sigma > 3: return -np.inf
            if mpeak < -2: return -np.inf
            if mpeak > 1: return -np.inf
