...
@dataclass
class RenderMap:
    required_datafields: set = {"PartType0/Coordinates", "PartType0/SmoothingLength"}
    plotlabel: str = ""
    colormap = "viridis"
    name = ""

    def render(self, pdata: dict, meshoid: Meshoid, mapargs: dict):
        return getattr(rendermaps, self.name).render(pdata, meshoid, mapargs)
...
#     def render_VelocityDispersion(self, pdata: dict, meshoid: Meshoid, mapargs: dict, vproj_LIC=True):
