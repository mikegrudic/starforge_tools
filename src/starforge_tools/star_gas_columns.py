...
def star_gas_columns(xstar, xgas, mgas, hgas, sightline_z=np.array([0, 0, 1.0])):
    """
    ...
    sightline_z: array_like, optional
    ...
    """
    if not np.allclose(
        sightline_z, np.array([0, 0, 1.0])
    ):  # have to remap coordinates if sightline is not along +z direction
        if not isinstance(sightline_z, np.ndarray):
            sightline_z = np.array(sightline_z, dtype=np.float64)
        sightline_z /= (sightline_z * sightline_z).sum() ** 0.5

        # generate a random x axis, deproject z component, then cross to get new y axis
        sightline_x = np.random.normal(size=(3,))  # 3D random
        sightline_x -= np.inner(sightline_x, sightline_z) * sightline_z  # deproject
        sightline_x /= (sightline_x * sightline_x).sum() ** 0.5  # normalize
        sightline_y = np.cross(sightline_z, sightline_x)  # cross product
        sightline_matrix = np.c_[sightline_x, sightline_y, sightline_z].T  # make unitary coordinate basis matrix
