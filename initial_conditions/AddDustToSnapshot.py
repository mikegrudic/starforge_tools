...
for f in argv[1:]:
    ...
    with h5py.File(f, "r") as F:
        ...
        for k in F.keys():
            F.copy(k, Fdust)
        Fdust["PartType0"]["Masses"].write_direct(
            (1 - mu) * np.array(Fdust["PartType0"]["Masses"])
        )
        Fdust.create_group("PartType3")
        Ngas = len(np.array(Fdust["PartType0"]["Coordinates"]))
        print(Ngas)
        Fdust["PartType3"].create_dataset(
            "Coordinates",
            data=np.array(Fdust["PartType0"]["Coordinates"])
            * (1 + 1e-6 * np.random.normal(size=(Ngas, 3))),
        )
        Fdust["PartType3"].create_dataset(
            "Velocities", data=np.array(Fdust["PartType0"]["Velocities"])
        )
        Fdust["PartType3"].create_dataset(
            "Masses", data=mu * np.array(Fdust["PartType0"]["Masses"])
        )
