...
stuff_to_skip = "StarFormationRate", "BH_Dist", "TurbulenceDissipation", "Vorticity", "VelocityDivergence", "DivergenceOfMagneticField", "DivBcleaningFunctionPhi", "DivBcleaningFunctionGradPhi"
...
def is_this_already_compressed(f):
    ...
    if "_bug" in f: return True
    if not "hdf5" in f: return True
    ...
    for k in F.keys():
        ...
        for k2 in F[k].keys():
            ...
            if k2 in stuff_to_store_at_half_precision:
                F2[k].create_dataset(k2, data=np.float16(np.log10(F[k][k2])), compression="gzip", fletcher32=True, shuffle=True, chunks=True)
            else:
                F2[k].create_dataset(k2, data=F[k][k2], compression="gzip", fletcher32=True, shuffle=True, chunks=True)

    F2.close()
    F.close()
    if overwrite: move(f2name, f)
    

def main():
    files_to_compress = []
    for root, dirs, files in walk('.'):
        if "stars_" in root: continue
        for f in files:
            if ".hdf5" in f and "snapshot" in f:
                if "_stars" in f: continue
                path = root+"/"+f
