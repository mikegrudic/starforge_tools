...
- Instead of editing ``Makefile.systype`` (which can be overwritten when you pull a new code version), you can put your ``SYSTYPE="..."`` line in a ``.gizmo`` file in your home directory — GIZMO checks ``~/.gizmo`` before falling back to ``Makefile.systype``, and a ``SYSTYPE`` environment variable overrides both.
