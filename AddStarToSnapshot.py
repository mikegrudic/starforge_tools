...
for f in snapshot_paths:
    ...
    with h5py.File(f,'r') as F:
        ...
        dist = cdist(Fstar["PartType0/Coordinates"][:], [xstar]).flatten(); closest = dist.argmin()
        hsml = F["PartType0/SmoothingLength"][:][closest]

        if options["--v"] == "comoving":
            vstar = F["PartType0/Velocities"][:][closest]
        else:
            vstar = np.array([float(s)*1e5 / F["Header"].attrs["UnitVelocity_In_CGS"] for s in options["--v"].split(",")])

        age = float(options["--age"]) * 3.1541e13 / (F["Header"].attrs["UnitLength_In_CGS"]/F["Header"].attrs["UnitVelocity_In_CGS"])
        ...
        Fstar["PartType5"].create_dataset("SinkRadius", data=[F["Header"].attrs["Fixed_ForceSoftening_Keplerian_Kernel_Extent"][5]])
